...
def register_routes(app, ctx):
    ...
    def add_proxy(req):
        ...
        ctx.store.save_settings(settings)
        return _ok(proxy=_proxy_out(proxy, ctx.store.load_nodes()))
    ...
    def update_proxy(req, pid):
        ...
        ctx.store.save_settings(settings)
        return _ok(proxy=_proxy_out(target, ctx.store.load_nodes()))
    ...
    def get_settings(req):
        settings = ctx.store.load_settings()
        return _ok(
            core_binary=settings.get("core", {}).get("binary") or ctx.core.binary,
    ...
    def put_settings(req):
        ...
        if "core_binary" in body:
            core["binary"] = str(body["core_binary"] or "")
        settings["core"] = core
        ctx.store.save_settings(settings)
